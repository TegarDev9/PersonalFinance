...
(async () => {
  try {
    ...
    info(
      `Counts: accounts=${accounts.length}, tx=${txs.length}, categories=${categories.length}, budgets=${budgets.length}, rules=${rules.length}`
    );
    ...
    for (const [name, count] of Object.entries(nameCounts)) {
      if (name && count > 1) {
        warn(`Duplicate account name "${name}" x${count}`);
        dupAccWarn++;
      }
    }
    ...
    if (dupErr || missBudCat || regexErr || ruleCatErr || orphanAccErr || typeSignErr) {
      error(
        `Validation failed. duplicateBudgets=${dupErr}, missingBudgetCategory=${missBudCat}, invalidRegex=${regexErr}, missingRuleCategory=${ruleCatErr}, orphanAccounts=${orphanAccErr}, typeSignMismatch=${typeSignErr}`
      );
      process.exit(1);
    }
